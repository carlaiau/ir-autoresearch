...
JASSJR_ABLATION_BM25_RM3_RM3EXP_DENSE_FUSION: $tri_source_expansion_output
EOF
...
if [[ "$openai_mode" == "off" ]]; then
  write_off_metadata
  cat "$tri_source_expansion_output" > "$final_results_file"
else
  python3 "$repo_root/tools/openai_rerank.py" \
    --repo-root "$repo_root" \
    --workdir "$workdir" \
    --topics-file "$topics_stdin_file" \
    --run-file "$tri_source_expansion_output" \
    --output-file "$final_results_file" \
    --metadata-file "$rerank_metadata_file"
  append_metadata "$rerank_metadata_file"
fi
