...
printf "%s\n" "$summary"
{
  ...
  if [[ -n "${JASSJR_BM25_K1:-}" || -n "${JASSJR_BM25_B:-}" || -n "${JASSJR_FEEDBACK_DOCS:-}" || -n "${JASSJR_EXPANSION_TERMS:-}" || -n "${JASSJR_EXPANSION_WEIGHT:-}" || -n "${JASSJR_EXPANSION_MAX_QUERY_TERMS:-}" || -n "${JASSJR_SEMANTIC_MODE:-}" || -n "${JASSJR_SEMANTIC_MODEL:-}" || -n "${JASSJR_SEMANTIC_DIMENSIONS:-}" || -n "${JASSJR_SEMANTIC_DOC_WORDS:-}" || -n "${JASSJR_SEMANTIC_BATCH_SIZE:-}" || -n "${JASSJR_SEMANTIC_TOPK:-}" || -n "${JASSJR_FUSION_RRF_K:-}" || -n "${JASSJR_FUSION_WEIGHT_BM25:-}" || -n "${JASSJR_FUSION_WEIGHT_RM3:-}" || -n "${JASSJR_FUSION_WEIGHT_DENSE:-}" || -n "${JASSJR_FUSION_WEIGHT_DENSE_BACKFILL:-}" || -n "${JASSJR_FUSION_BM25_TOPK:-}" || -n "${JASSJR_FUSION_RM3_TOPK:-}" || -n "${JASSJR_FUSION_DENSE_TOPK:-}" || -n "${JASSJR_FUSION_DENSE_BACKFILL_TOPK:-}" || -n "${JASSJR_RERANK_DOCS:-}" || -n "${JASSJR_RERANK_PASSAGE_WINDOW:-}" || -n "${JASSJR_RERANK_PASSAGE_WEIGHT:-}" || -n "${JASSJR_OPENAI_RERANK_MODE:-}" || -n "${JASSJR_OPENAI_MONO_MODEL:-}" || -n "${JASSJR_OPENAI_DUO_MODEL:-}" || -n "${JASSJR_OPENAI_MONO_DOCS:-}" || -n "${JASSJR_OPENAI_DUO_DOCS:-}" || -n "${JASSJR_OPENAI_DOC_WORDS:-}" || -n "${JASSJR_OPENAI_PROMPT_VERSION:-}" || -n "${JASSJR_OPENAI_CACHE_DIR:-}" ]]; then
    printf "\n"
  fi
